...
export async function seedCategories(): Promise<void> {
  for (const cat of CATEGORIES) {
    ...
    if (existingQuestions > 0) {
      console.log(`Category ${cat.name} already has ${existingQuestions} questions, skipping`);
      continue;
    }

    const questions = QUESTIONS_BY_CATEGORY[cat.slug] || [];
    ...
    console.log(`Seeded ${questions.length} questions for category: ${cat.name}`);
  }
}
